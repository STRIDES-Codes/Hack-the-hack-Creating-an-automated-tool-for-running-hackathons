...
def random_teams(auth_keys, fields, rows, num_teams):
    ...
    final_object = {
        "host": {
            "github_auth_key": auth_keys[0],
            "drive_auth_key": auth_keys[1]
        },
        "teams": [
...
def make_teams(auth_keys, fields, rows):
    # JSON object that stores everything
    final_object = {
        "host": {
            "github_auth_key": auth_keys[0],
    ...
    if "project" in fields:
        ...
        # no one was actually assigned a project, teams will be randomized
            ...
            final_object = random_teams(auth_keys, fields, rows, num_teams)
...
def create_github_repos(info_dict):
    ...
    for (num,team) in enumerate(info_dict["teams"]):
        try:
            repo = host_user.create_repo(team["team_name"])
            print(f"Created repo {repo.full_name}", file=sys.stderr)
            for member in team["members"]:
                try:
                    repo.add_to_collaborators(member['github_username'])
                    print(f"Added user {member} to {repo.full_name}", file=sys.stderr)
                except:
                    print(f"User {member} does not correspond to a GitHub account", file=sys.stderr)
            print(f"Link to repo: {repo.html_url}", file=sys.stderr)
            info_dict["teams"][num]["github_repo"] = repo.html_url
        except:
